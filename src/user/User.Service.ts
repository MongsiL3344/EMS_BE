...
import type { LoginDto, LoginResult, UserServiceInterface, UserRepositoryInterface, SessionCheckResult, SessionInfo} from "../types/User.Type.js";
...
export class UserService implements UserServiceInterface {
  ...
  public async login (userInputData: LoginDto): Promise<LoginResult> {
    try {
      ...
      if (user == null) {
        console.error("[SERVICE] USER_NOT_FOUND");
        return {
          ok: false,
      ...
      if (!pwCompare) { //비밀번호가 맞지 않으면 콘솔에러 -> false 리턴
        console.error("[SERVICE] PASSWORD_MISMATCH");
        return {
          ok: false,
    ...
    } catch (error) { // 세션생성 실패 -> 콘솔에러, 입력정보+false 리턴
  ...
  public async logout (userId: number): Promise<void> {
    await this.userRepository.clearSession(userId); //로그아웃시 세션청소
  }
  ...
  public async checkSessionBySid  (sid: string): Promise<SessionCheckResult> {
    ...
    return { state: "valid", user: { id: user.id, email: user.email, session_id: user.session_id, session_expires_at: user.session_expires_at} } };
}
